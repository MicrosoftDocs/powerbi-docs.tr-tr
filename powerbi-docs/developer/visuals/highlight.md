...
Gereksiz kod kaldırıldıktan sonra varsayılan görsel kaynak kodu şöyle görünür:
...
Power BI’dan veri işlemek için gerekli arabirimleri içeri aktarın:
...
Kategori değerleri için kök `div` öğesini oluşturun:
...
Yeni verileri işlemeden önce div öğelerinin içeriğini temizleyin:
...
`dataView` nesnesinden kategorileri ve ölçü değerlerini alın:
...
`categoryValues` dizisini listeleyin ve karşılık gelen değerleri ve vurguları alın:
...
Görsel DOM içindeki veri görünümü değerlerini görüntüleyip görselleştirmek için `div` ve `p` öğelerini oluşturun:
...
`flex box` kullanmak ve div öğelerinin renklerini tanımlamak için öğelere gerekli stilleri uygulayın:
...
Matris veri görünümü eşlemesi için hiyerarşi oluşturulacak örnek veriler:

|   Row1   |   Row2   |   Row3   |   Sütun1   |   Sütun2   |   Sütun3   |   Değerler   |
|-----|-----|------|-------|-------|-------|-------|
...
Varsayılan görsel projesini oluşturun ve `capabilities.json` örneğini uygulayın.

Gereksiz kod kaldırıldıktan sonra varsayılan görsel kaynak kodu şu şekilde görünür:
...
Görsel düzeni için iki `div` öğesi oluşturun:
...
Görselin verileri aldığından emin olmak için `update` yöntemindeki verileri denetleyin:
...
Yeni veriler oluşturmadan önce `div` öğelerinin içeriğini temizleyin:
...
Matris veri yapısına çapraz geçiş için `treeWalker` işlevi oluşturun:
...
Burada `matrixNode` geçerli düğüm, `levels` bu hiyerarşi düzeyinin meta veri sütunları, `div` - alt HTML öğelerinin üst öğesidir.

`treeWalker` bir özyinelemeli işlevdir, üst bilgi olarak metin için `div` ve `p` öğesini oluşturması ve düğümün alt öğeleri için işlevi çağırması gerekir:
...
Sütunun kök öğeleri ve matris veri görünümü yapısının satırı için işlevi çağırın:
...
Düğümler için selectionID ve düğümleri görüntülemek için Oluştur düğmeleri oluşturun:
...
Vurgulamayı kullanmanın ana adımı, ek değer dizisi işlemektir.

Terminal düğümünün nesnesini inceliyorsanız, değerler dizisinin değer ve vurgulama olarak iki özelliği olduğunu görebilirsiniz:
...
Gösteriliyorsa `values` düğüm dizisini işlemek için kod ekleyin:
...
* [Matris veri görünümü eşlemeleri hakkında bilgi edinin](dataview-mappings.md#matrix-data-mapping)

* [Görselin özellikleri hakkında bilgi edinin](capabilities.md)
