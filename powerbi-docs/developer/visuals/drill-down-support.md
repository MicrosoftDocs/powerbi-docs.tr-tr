...
## <a name="enable-drill-down-support-in-the-visual"></a>Görselde detaya gitme desteğini etkinleştirme
...
> dataRole koşullarındaki `max` özelliği 1 olarak ayarlanmalıdır.

Detaya gitmeye rolü eklediğinizde, kullanıcılar veri rolüne birden çok alanı sürükleyebilir.
...
## <a name="create-the-visual-with-drill-down-support"></a>Detaya gitme desteği olan görseli oluşturma

Varsayılan örnek görseli oluşturmak için

```cmd
pbiviz new testDrillDown -t default
```

komutunu çalıştırın. Yukarıdaki örneğin `capabilities.json` dosyasını yeni oluşturulan görsele uygulayın.

Görselin HTML öğelerini tutmak üzere `div` kapsayıcısının özelliğini oluşturun:
...
Görselin oluşturucusunu güncelleştirin:
...
`.\style\visual.less` konumundaki basit stilleri uygulayın:
...
Görseli test etmek için örnek verileri hazırlayın:
...
`forEach` işlevi geri çağrısının gövdesini şöyle değiştirin:
...
Görseli matris veri görünümü eşlemeleriyle test etmek için örnek verileri hazırlayın:
...
Görsel için aşağıdaki veri görünümü eşlemesini uygulayın:
...
Satır ve sütun öğelerinin iki `div` öğesi için iki özellik oluşturun:
...
Hiyerarşide geçiş yapmak için `treeWalker` işlevi oluşturun:
...
Veri noktaları için seçimleri oluşturun.
...
Her hiyerarşi düzeyi için `div` oluşturun:
...
Görselle etkileşim kurmak ve matris veri noktalarında bağlam menüsünü görüntülemek için `buttons` oluşturun:
...
Öğeleri yeniden işlemeden önce `div` öğelerini temizleyin:
