...
Denetim, Power BI Pro özelliğidir ve olayları denetleme işlevi yalnızca Power BI Pro kullanıcıları tarafından kullanılabilir.  Power BI (ücretsiz) lisansına sahip kullanıcılar **Ücretsiz Kullanıcı** olarak görüntülenir.

Kuruluşunuzdaki kullanıcılar için Power BI Pro lisansı satın alma ve atama hakkında daha fazla bilgi için bkz. [Power BI Pro'yu satın alma](service-admin-purchasing-power-bi-pro.md).

Ücretsiz kullanıcıların kaydolmasını sınırlama hakkında daha fazla bilgi için bkz. [Azure Active Directory'de bireysel kullanıcı kaydını etkinleştirme veya devre dışı bırakma](service-admin-service-free-in-your-organization.md#enable-or-disable-individual-user-sign-up-in-azure-active-directory).
...
| **Parametre** | **Açıklama** |
| --- | --- |
| Id |Denetim kaydının benzersiz tanımlayıcısı. |
| RecordType |Kayıtta belirtilen işlem türü. Günlük kaydı türleri hakkında ayrıntılı bilgi için AuditLogRecordType tablosuna bakın. |
| CreationTime |Kullanıcının etkinliği gerçekleştirdiği tarih ve Eşgüdümlü Evrensel Saat (UTC) biçiminde saat. |
| Operation |Kullanıcı veya yönetici etkinliğinin adı. |
| OrganizationId |Olayın gerçekleştiği, kuruluşunuzun Office 365 hizmetinin GUID değeri. |
| UserType |İşlemi gerçekleştiren kullanıcının türü. Kullanıcı türleri hakkında ayrıntılı bilgi için Kullanıcı Türü tablosuna bakın. |
| UserKey |Etkinliği gerçekleştiren kullanıcının Passport Benzersiz Tanımlayıcısı değeri. |
| ResultStatus |Eylemin (Operation özelliğinde belirtilen) başarılı olup olmadığını belirtir. Olası değerler: Succeeded, PartiallySucceded veya Failed. |
| ObjectId |SharePoint veya OneDrive İş etkinliği için kullanıcı tarafından erişilen dosyanın veya klasörün tam yolu. |
| UserId |Kaydın günlüğe kaydedilmesine neden olan eylemi (Operation özelliğinde belirtilen) gerçekleştiren kullanıcının UPN (Kullanıcı Asıl Adı) değeri; örneğin, my_name@my_domain_name. Sistem hesapları (SHAREPOINT\system veya NT AUTHORITY\SYSTEM gibi) tarafından gerçekleştirilen eylemlerle ilgili kayıtlar da dahil edilir. |
...
Aşağıdaki tabloda görebileceğiniz olaylarla ilgili ayrıntılar verilmiştir.

| **Olay** | **Açıklama** | **Ek Bilgiler** |
| --- | --- | --- |
| Power BI raporu indirme |Bu etkinlik her rapor indirildiğinde günlüğe kaydedilir |Rapor Adı, Veri Kümesi Adı |
...
| Power BI uzatılmış deneme sürümünü başlatma |Bu etkinlik bir kullanıcı 31 Mayıs 2018'e kadar devam edecek uzatılmış Pro deneme sürümünü kabul ettiğinde günlüğe kaydedilir | |
...
| Power BI ağ geçidi yöneticilerinin değiştirilmesi |Bu etkinlik bir ağ geçidinin yöneticileri değiştirildiğinde (ekleme/kaldırma yapıldığında) günlüğe kaydedilir |Ağ Geçidi Adı, Eklenen Kullanıcılar, Kaldırılan Kullanıcılar |
| Power BI ağ geçidi veri kaynağı kullanıcılarının değiştirilmesi |Bu etkinlik bir ağ geçidinin kullanıcıları değiştirildiğinde (ekleme/kaldırma yapıldığında) günlüğe kaydedilir |Ağ Geçidi Adı, Eklenen Kullanıcılar, Kaldırılan Kullanıcılar |
